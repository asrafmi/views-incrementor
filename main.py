...
def increase_views(url, num_views):
    for i in range(num_views):
        try:
            headers = {'User-Agent': get_random_user_agent()}
            response = requests.get(url, headers=headers)
            print(f"{i + 1}. Response: {response.status_code}")
            print(f"Visited by {headers['User-Agent']}")
        except requests.RequestException as e:
            print(f"An error occurred: {e}")
            print("Retrying...")
            continue
        time.sleep(1)  # delay untuk mencegah spamming

if __name__ == "__main__":
    url_to_increase_views = input("Masukkan URL yang ingin ditingkatkan views-nya: ")
    views = int(input("Masukkan jumlah views yang ingin ditambahkan: "))
    increase_views(url_to_increase_views, views)
